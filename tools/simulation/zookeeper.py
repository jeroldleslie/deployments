...
class ZookeeperTool(object):
  ...
  def forceStop(self, limitHosts):
    self.ansibleRunner.deploy(playbook = self.zookeeperPlaybook, inventory=self.inventoryFile, outputToStdout=True, maxRetries=0, tags="force-stop", limit=limitHosts);

  def start(self, limitHosts):
    self.ansibleRunner.deploy(playbook = self.zookeeperPlaybook, inventory=self.inventoryFile, outputToStdout=True, maxRetries=0, tags="start", limit=limitHosts);

  def clean(self, limitHosts):
    self.ansibleRunner.deploy(playbook = self.zookeeperPlaybook, inventory=self.inventoryFile, outputToStdout=True, maxRetries=0, tags="clean", limit=limitHosts);
