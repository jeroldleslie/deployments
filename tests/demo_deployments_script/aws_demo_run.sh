...
while true; do
   ...
   curl  -X POST $gripper_host/message/test-topic1/sync -d '{"event": {"id":'$counter',"type":"'$type'","ts": '$ts', "hmac": "'$hmac'" }} '
   echo 'Sending message : {"event": {"id":'$counter',"type":"'$type'","ts": '$ts',"date":"'$date_ts'", "hmac": "'$hmac'" }} '
#   sleep 1
done

# Run this file multiple times parallely
#for((i=1;i<100;i++)); do nohup ./aws_demo_run.sh ec2-52-36-174-224.us-west-2.compute.amazonaws.com & done
