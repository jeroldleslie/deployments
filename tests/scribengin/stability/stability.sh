...
ssh -o "StrictHostKeyChecking no" neverwinterdp@hadoop-master "chmod +x /opt/neverwinterdp/dataflow/log-sample-stability/bin/*.sh"
...
#KILL_WORKER_OPTS="--kill-worker-random=true --kill-worker-period=90000 --kill-worker-max=25"

ssh -o "StrictHostKeyChecking no" neverwinterdp@hadoop-master \
   "cd /opt/neverwinterdp &&  time ./dataflow/log-sample/bin/run-dataflow-chain-stability.sh $STORAGE_OPTS $DATAFLOW_OPTS $KILL_WORKER_OPTS"
