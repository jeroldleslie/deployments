...
function runTest() {
  time $NEVERWINTERDP_BUILD/scribengin/bin/tracking/run-tracking.sh $GENERATOR_OPTS $DATAFLOW_OPTS $VALIDATOR_OPTS 
}


function runWithFailureTest() {
  time $NEVERWINTERDP_BUILD/scribengin/bin/tracking/run-tracking-with-simulation.sh $GENERATOR_OPTS $DATAFLOW_OPTS $VALIDATOR_OPTS $SIMULATION_OPTS
}
